  ...
  tabItems(
      ...
      fluidRow(
        box(width = 12,
            column(6,
             ...
             column(6,
                    #conditionalPanel(condition = "input.format == NA ",
    ...
    tabItem(tabName = "soci",
            ...
            fluidRow(
              box(width = 12,
                  column(6,
                         radioButtons('format2', 'Select a Document Format', c('PDF', 'HTML'), inline = TRUE, selected = NA)
                  ),
                    column(6,
                         downloadButton('SocialReport', 'Generate Social Report') 
                  ))),
            fluidRow(
              box(width = 6,
                  fluidRow(
    ...
    tabItem(tabName = "econ",
            ...
            fluidRow(
              box(width = 12,
                  column(6,
                         radioButtons('format3', 'Select a Document Format', c('PDF', 'HTML'), inline = TRUE, selected = NA)
                  ),
                  column(6,
                         downloadButton('EconomicsReport', 'Generate Economics Report') 
                  ))),
            fluidRow(
              box(width = 6,
...
server <- function(input, output, session){
  ...
  place <- reactive({
    ...
    my_place
      ...
      library(knitr)
      shinyjs::useShinyjs()
      #output$downloadReport2 <- renderUI({
        #req(input$format)
        #downloadButton("downloadReport")
      #})
  
  #observeEvent(downloadReport, {
    # When the button is clicked, wrap the code in a call to `withBusyIndicatorServer()`
    #withBusyIndicatorServer("downloadReport", {
      #Sys.sleep(1)
      #if (input$format == "") {
        #stop("select a format")
      #}
    #})
    ...
    #})
      ...
      observe({
        toggleState("downloadReport", !is.null(input$format) && input$format != "")
      })
      ...
      output$downloadReport <- downloadHandler(
        ...
        #validate(
          #need(input$format!= "", "Select a Document Format")),
        
        
      #observeEvent(input$format, {
      
          #if (input$format == "") {
            #shinyjs::disable("downloadReport") #, "Select a Document Format"
          #} else {
            #shinyjs::enable("downloadReport") #, "Generate Demographics Report"
          #}
        #}),
        
        filename = function() {
          paste('Demographics_Report', sep = '.', switch(
            input$format, PDF = 'pdf', HTML = 'html'
          ))
        },
      
        content = function(file) {
          src <- normalizePath('Demographics_report.Rmd')
          src2 <- normalizePath('logo.jpg') 
          # temporarily switch to the temp dir, in case you do not have write
          # permission to the current working directory
          owd <- setwd(tempdir())
          on.exit(setwd(owd))
          file.copy(src, 'Demographics_report.Rmd', overwrite = TRUE)
          file.copy(src2, 'logo.jpg') 
          library(rmarkdown)
          out <- render('Demographics_report.Rmd', switch(
            input$format,
      ...
      output$SocialReport <- downloadHandler(
        
        filename = function() {
          paste('Social_Report', sep = '.', switch(
            input$format2, PDF = 'pdf', HTML = 'html'
          ))
        },
        
        content = function(file) {
          src3 <- normalizePath('Social_report.Rmd')
          src4 <- normalizePath('logo.jpg') 
          # temporarily switch to the temp dir, in case you do not have write
          # permission to the current working directory
          owd <- setwd(tempdir())
          on.exit(setwd(owd))
          file.copy(src3, 'Social_report.Rmd', overwrite = TRUE)
          file.copy(src4, 'logo.jpg') 
          library(rmarkdown)
          out2 <- render('Social_report.Rmd', switch(
            input$format2,
            PDF = pdf_document(), HTML = html_document()
          ))
      ...
      output$EconomicsReport <- downloadHandler(
        
        filename = function() {
          paste('Economics_report', sep = '.', switch(
            input$format3, PDF = 'pdf', HTML = 'html'
          ))
        },
        
        content = function(file) {
          src5 <- normalizePath('Economics_Report.Rmd')
          src6 <- normalizePath('logo.jpg') 
          # temporarily switch to the temp dir, in case you do not have write
          # permission to the current working directory
          owd <- setwd(tempdir())
          on.exit(setwd(owd))
          file.copy(src5, 'Economics_Report.Rmd', overwrite = TRUE)
          file.copy(src6, 'logo.jpg') 
          library(rmarkdown)
          out3 <- render('Economics_Report.Rmd', switch(
            input$format3, 
            PDF = pdf_document(), HTML = html_document()
          ))
