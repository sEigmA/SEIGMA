...
body <- dashboardBody(
  ...
  tabItems(
    tabItem(
      ...
      fluidRow(
        box(width = 12,
            a(img(src = "logo.jpg", height="25%", width="100%"), href="http://www.umass.edu/seigma/", target = "_blank")
        )
      ),
      fluidRow(
        box(width = 12,
            column(6,
                    radioButtons('format', 'Select a Document Format', c('PDF', 'HTML'), inline = TRUE)
                   ),
             column(6,
    ...
    tabItem(tabName = "soci",
            ...
            fluidRow(
              box(width = 12,
                  column(6,
                         radioButtons('format2', 'Select a Document Format', c('PDF', 'HTML'), inline = TRUE)
                  ),
                    column(6,
    ...
    tabItem(tabName = "econ",
            ...
            fluidRow(
              box(width = 12,
                  column(6,
                         radioButtons('format3', 'Select a Document Format', c('PDF', 'HTML'), inline = TRUE)
                  ),
                  column(6,
...
server <- function(input, output, session){
  ...
  place <- reactive({
    ...
    my_place
      ...
      #observeEvent(input$format!= "", "Generate Demographics Report"),
      
          #if (input$format == "") {
            #shinyjs::disable("Generate Demographics Report")
          #} else {
            #shinyjs::enable("Generate Demographics Report")
          #},
