...
url<-"http://www2.census.gov/geo/tiger/TIGER2010DP1/CouSub_2010Census_DP1.zip"
downloaddir<-getwd()
destname<-"tiger.zip"
...
filename<- "CouSub_2010Census_DP1"
...
names(subdat)[names(subdat) == "NAMELSAD10"]<-"Municipal"

#Simplify shape file
# ----- save the data slot
subdat_data<-subdat@data[,c("GEOID10", "Municipal", "Population")]
...
save(subdat, file = "muni_subdat.RData")
...
######################################################################
#Starting from here? load in the data:
