...
shinyServer(function(input, output, session){
  ...
  output$summary <- renderDataTable({
    ...
    if(input$sum_timespan == "sing.yr"){
      df <- filter(pValue_df, Year==input$sum_year)
    }
    ...
    if(input$sum_timespan == "mult.yrs"){
      ...
      for(i in 1:length(range)){
        bbb <- subset(pValue_df, Year==range[i])
  ...
  plot_dat<- reactive({
    ...
    ##col_sel_num<-c()
    if (input$plot_radio == "Total_Assessed") {
      ## make region a vector based on input variable
      munis <- input$plot_muni
      col<-input$plot_display_radio
      col_sel_num<-which( colnames(pValue_df)==col )
    }
    else {
      ## make region a vector based on input variable
      munis <- input$plot_muni2
      col_sel_num<-c(25,26,28:30)
    }
    
    ## assign column to each entry in the data frame
    ##col_sel_num1<-which( colnames(pValue_df)==col )
    plot_dat <- pValue_df %>%
      filter(Municipal %in% munis) %>%
      select(1, 4, col_sel_num)
  ...
  ## for the Google charts plot (Total pValue plot)
  output$pValue_plot1 <- reactive({
    ## make reactive dataframe into regular dataframe
    if (input$plot_radio == "Total_Assessed"& input$plot_display_radio=="Total_Assessed_Million"){
    TotpValue_df <- plot_dat()
    TotpValue_df<-TotpValue_df%>%
      spread("Municipal", "Total_Assessed_Million")
    
      list(
        data=googleDataTable(TotpValue_df))  
    }
     })
  
  ##change in Total pValue Plot
  output$pValue_plot2 <- reactive({
    if (input$plot_radio == "Total_Assessed"& input$plot_display_radio=="Total_Assessed_Pct_Change"){
    ## make reactive dataframe into regular dataframe
    TotpValue2_df <- plot_dat()
    TotpValue2_df<-TotpValue2_df%>%
      spread("Municipal", "Total_Assessed_Pct_Change")
    list(
      data=googleDataTable(TotpValue2_df))
    }
  })
  
  output$pct_plot1 <- reactive({
    if (input$plot_radio == "Percent_Assessed") {
    ## make reactive dataframe into regular dataframe
    pct_df <- plot_dat()
    ymax<-max(pct_df[,3])+2
    ...
    list(
      ...
      options = list(
        ...
        vAxis = list(
          title = "Assessed Property Values in $Millions",
          viewWindow = ymax,
          textStyle = list(
            fontSize = font_size),
          titleTextStyle = list(
  ...
  map_dat <- reactive({
    ...
    col_sel_num1<-which( colnames(pValue_df)==col )
    map_dat <- select(pValue_df,c(1,4,col_sel_num1))
    ...
    missing.munis <- setdiff(leftover_munis_map,pValue_df$Municipal)
    ...
    if (input$map_radio == "Total_Assessed" & input$map_display_radio=="Total_Assessed_Pct_Change"){
  ...
  output$details <- renderText({
    ...
    if(input$map_display_radio == "Total_Assessed_Pct_Change"|| input$map_radio == "Percent_Assessed"){
    ...
    if(muni_value == "NULL"){
      return(as.character(tags$div(
        tags$h5("Annual Total Assessed Property Values for", muni_name, "is not available for this timespan"))))
    }
    ...
    if (input$map_radio == "Total_Assessed") {
      ...
      return(as.character(tags$div(
        tags$h4("Change in Total Assessed Property Values in", muni_name, " for ", input$map_year,"since year 2003"),
        tags$h5(muni_value,"%")
      )))
    }
  }
  ...
  output$legend3<- renderPlot({
    if(input$map_radio == "Total_Assessed"&input$map_display_radio == "Total_Assessed_Pct_Change"){
      ...
      cols <- paint.brush(25)
      leg_dat <- data_frame(y = seq(pValueChamin.val, pValueChamax.val,length.out=(length(map_colors1)-1)), x = 1, col = cols)
      
      b <- ggplot(data = leg_dat) +
        geom_tile(aes(y = y, fill = reorder(col, y), x = x), show.legend = FALSE) +
        scale_y_continuous(limits = c(pValueChamin.val, pValueChamax.val), breaks = seq(pValueChamin.val, pValueChamax.val, length.out = 5)) +
        scale_fill_manual(values = leg_dat$col) + theme_bw() +
        theme(axis.text.x = element_blank(),
              axis.text.y = element_text(size = 12),
              axis.title.x = element_blank(),
  ...
  output$legend2 <- renderPlot({  
    if(input$map_radio == "Percent_Assessed"){
      paint.brush = colorRampPalette(colors=c("white", "violetred"))
