...
setwd("~/Documents/R/SEIGMA/Updating Shiny Data/buildingpermits")
...
CPI <- read.csv("CPI_2000_2019.csv", stringsAsFactors = FALSE)
...
CPI_2019 <- as.numeric(CPI[which(CPI$year == 2019),2]) #get CPI for current year
est_2010 <- read.csv("sub-est00int.csv", stringsAsFactors = FALSE) #population counts before 2010
est_2019 <- read.csv("sub-est2019_25.csv", stringsAsFactors = FALSE) #MA population counts after 2010
...
#   pre_year <- read_excel("building_permits_2000-2019.xlsx", sheet = sheet_ind, col_names = F)
...
allsheets <- read_excel_allsheets("building_permits_2000-2019.xlsx")
...
buildingpermits_A <- buildingpermits %>% filter(id %in% c("2019", "2018", "2017", "2016", "2015", "2013", "2012"))
...
for (i in 1:nrow(buildingpermits)){
  ...
  buildingpermits$Inflation_Adjusted_1_Family_Valuation[i] <- (CPI_2019/cpi_pre) * buildingpermits$Single_Family_Valuation[i]
  buildingpermits$Inflation_Adjusted_2_Family_Valuation[i] <- (CPI_2019/cpi_pre) * buildingpermits$I2_Family_Valuation[i]
  buildingpermits$Inflation_Adjusted_3_4_Family_Valuation[i] <- (CPI_2019/cpi_pre) * buildingpermits$'I3-4_Family_Valuation'[i]
  buildingpermits$Inflation_Adjusted_5_Family_Valuation[i] <- (CPI_2019/cpi_pre) * buildingpermits$I5_Family_Valuation[i]
  buildingpermits$Inflation_Adjusted_Total_Valuation[i] <- (CPI_2019/cpi_pre) * buildingpermits$Total_Valuation[i]
  buildingpermits$Inflation_Adjusted_Average_Valuation[i] <- (CPI_2019/cpi_pre) * buildingpermits$Average_Valuation_PerUnits[i]
}
...
est_2019 <- est_2019[!duplicated(est_2019$NAME), c(9,13:21)] #unique values
est_2019 <- est_2019[!grepl("Balance of", est_2019$NAME),]
#est_2019$NAME[!est_2019$NAME %in% est_2010$NAME] #to find mismatches
pop_est <- cbind(est_2010, est_2019) #merge full population data
...
# save dataset to folder
 write.csv(buildingpermits, "buildingdata_updated_2019.csv")
